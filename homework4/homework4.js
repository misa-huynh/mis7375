...
const TIME_UNTIL_WARNING_SEC = 30;
const WARNING_SEC = 30;

let warningTimer, logoutCounter, sessionTimer;

setInterval(updateTime, 6000);

// Happens every time we load the page
document.addEventListener("DOMContentLoaded", () => {
  const displayName = document.getElementById("display-name");
  const fNameInput = document.getElementById("fname");
  const firstName = "firstName";

  const saveName = getCookie(firstName);
  updateTime();

  //cookie exist flow
  if (saveName != "") {
    let isThisYou = confirm("Are you " + saveName + "? Click 'Cancel' if not.");
    if (!isThisYou) {
      // They hit cancel
      setCookie("firstName", "", -1);
      inputList.forEach((line) => {
        const lineIdValue = line.id;
        localStorage.removeItem(lineIdValue);
      });
    } else {
      // they hit OK
      displayName.textContent = saveName;
      fNameInput.value = saveName;
    }
  }
  ...
  inputList.forEach((line) => {
    // Prefill textboxes
    const lineIdValue = line.id;
    const inputHTMLElement = document.getElementById(lineIdValue);
    let localStorageData = localStorage.getItem(lineIdValue);
    if (
      (inputHTMLElement.type == "checkbox" ||
        inputHTMLElement.type == "radio") &&
      localStorageData == "true"
    ) {
      inputHTMLElement.checked = true;
    } else {
      if (localStorageData != "") {
        inputHTMLElement.value = localStorageData;
      }
    }

    //add EventListeners to textboxes
    inputHTMLElement.addEventListener("input", () => {
      if (
        inputHTMLElement.type == "checkbox" ||
        inputHTMLElement.type == "radio"
      ) {
        localStorage.setItem(lineIdValue, inputHTMLElement.checked);
      } else {
        localStorage.setItem(lineIdValue, inputHTMLElement.value);
      }
    });
  });
...
function resetTimers() {
  ...
  warningTimer = setTimeout(() => {
    ...
    startCountdown();
  }, TIME_UNTIL_WARNING_SEC * 1000);
}

function stayConnected() {
  document.getElementById("session-warning").style.display = "none";
  clearInterval(logoutCounter);
  resetTimers();
}

function startCountdown() {
  let t = WARNING_SEC;
  document.getElementById("countdown-timer").innerText = t;
  logoutCounter = setInterval(() => {
    t--;
    document.getElementById("countdown-timer").innerText = t;
    if (t <= 0) {
      clearInterval(logoutCounter);
...
function forceLogout() {
  ...
  document.getElementById("popup-session-timeout").style.display = "block";
}

function setLocalStorage(key, value) {
  localStorage.setItem("fname", "Jason");
}
...
function setCookie(cname, cvalue, expire) {
  ...
  expirationDate.setTime(
    expirationDate.getTime() + expire * 24 * 60 * 60 * 1000,
  );
...
function getCookie(cname) {
  ...
  for (let i = 0; i < ca.length; i++) {
    ...
    // if name is found return the value
    if (c.indexOf(name) == 0) {
...
function setDisplayName() {
  ...
  var fnameValue = fname.value;
  if (fNameValidation() && rememberMeCheckBox.checked) {
    setCookie("firstName", fnameValue, 1);
  }
}
...
function handleOnClickRadio(name) {
  if (name == "Gender") {
    genderValidation();
  }
  ...
  if (name == "Vaccination Status") {
    vaccinatedValidation();
  }
  const listOfRadioButtonsWithSameName = document.getElementsByName(name);
  listOfRadioButtonsWithSameName.forEach((radioButton) => {
    localStorage.removeItem(radioButton.id);
  });
}

async function handleZipcodeOnblur() {
  const zipcodeInput = document.getElementById("zipcode");
  const cityInput = document.getElementById("city");
  const stateInput = document.getElementById("state");

  if (zipcodeValidation()) {
    let responseFromAPI = await getStateAndCityFromZipcode(zipcodeInput.value);
    let places = responseFromAPI["places"];
    let place = places[0];
    let city = place["place name"];
    let state = place["state abbreviation"];
...
function updateTime() {
  ...
  if (time) {
    ...
    time.innerText = new Date().toLocaleString("en-US", options);
  }
}
